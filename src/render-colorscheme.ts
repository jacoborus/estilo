function printAnsicolors (term) {
  return Object.keys(term).map((color, i) => `let g:terminal_ansi_colors_${color} = '${term[i]}'
`)
}

function printTermColors (term) {
  return Object.keys(term).map((color, i) => `let g:terminal_color_${color} = '${term[i]}'
`)
}

export default function (theme, pkg, date: string, term, estiloVersion: string): string {
  return `" ===============================================================
" ${theme.name}
" ${theme.description}
" URL: ${pkg.url}
